...
def run_trading_cycle_for_symbol(symbol: str):
    ...
    try:
        ...
        if raw_regime == "chaotic":
            logging.warning("Market is chaotic. No new trades will be placed.")
            return
        ...
        if pd.notna(atr_rolling_mean) and current_atr > atr_rolling_mean * 1.5:
            required_confidence = base_confidence + 0.1
        else:
            required_confidence = base_confidence

        if signal == 0 or confidence < required_confidence:
            logging.info(f"Filter 1: Signal is neutral or low confidence {confidence:.3f} (Required: {required_confidence:.3f})")
            return
        logging.info(f"Filter 1: AI signal {signal_str} with confidence {confidence:.3f} (Required: {required_confidence:.3f})")

        if not filters.mta_filter(latest_data['close'].iloc[0], latest_data['ema_200_1h'].iloc[0], signal):
            logging.warning("Filter 0: MTA filter failed. Signal rejected.")
            return
        logging.info("Filter 0: MTA filter passed.")

        try:
            sentiment_bytes = redis_client.get("news_sentiment_score")
            sentiment_score = float(sentiment_bytes) if sentiment_bytes else 0.0

            alpha = 0.1
            direction = 1 if signal == 1 else -1
            confidence_adjustment = alpha * sentiment_score * direction
            adjusted_confidence = max(0.0, min(1.0, confidence + confidence_adjustment))

            logging.info(
                f"Filter 2: Sentiment={sentiment_score:.2f}, adj={confidence_adjustment:+.3f}, "
                f"confidence {confidence:.3f} -> {adjusted_confidence:.3f}"
            )

            if adjusted_confidence < required_confidence:
                logging.warning("Filter 2: Adjusted confidence below threshold. Signal rejected.")
                return
            confidence = adjusted_confidence
        except (ValueError, TypeError):
            logging.warning("Filter 2: Could not parse sentiment score, skipping adjustment.")
               
        logging.info("Filter 2: Sentiment check passed.")
        ...
        if not filters.volatility_filter(current_atr, atr_rolling_mean):
            logging.warning("Filter 4: Volatility too high")
            return
        ...
        stop_loss_price = risk_management.calculate_stop_loss(
            current_price, 
            latest_data['atr'].iloc[0], 
            side
        )
